...
def security_next_items(active):
    ...
    progress = '<div class="security-choice-progress" aria-hidden="true"><span><img src="/passwork-assets/security/progress.svg" alt=""></span></div>'
    links = ''.join(f'<button type="button" class="security-next-choice" data-feature-index="{(active + step) % len(titles)}">{titles[(active + step) % len(titles)]}</button>' for step in (1, 2))
    return f'<div class="security-choice-footer">{progress}<div class="security-next-choices">{links}</div></div>'
...
{image('img1PxDots8PxPitch800600Source','','scene-dots')}
...
<article class="security-choice" aria-hidden="true"><p class="feature-label">Размещение</p><h3>Размещение внутри инфраструктуры</h3><p class="security-choice-description">Пассворк разворачивается на серверах компаний и хранит данные внутри её инфраструктуры. Без телеметрии, внешних API и зависимости от облачных провайдеров</p>{security_next_items(1)}</article>
<article class="security-choice is-active" aria-hidden="false"><p class="feature-label">Сертификация</p><h3>ГОСТ-шифрование</h3><p class="security-choice-description">Поддержка ГОСТ Р 34.10-2012 и ГОСТ Р 34.11-<br class="security-desktop-break">2012. Работа в закрытом контуре на серверах компании <br class="security-desktop-break">без зависимости от зарубежных сервисов</p>{security_next_items(2)}</article>
</div>
...
<span>FROM</span> deals
<span>WHERE</span>
  (
    -- Closed-Won this week (week of June 15–21, 2026)
    (closed_won_date &gt;= '2026-06-15' AND closed_won_date &lt;= '2026-06-21')</pre>
